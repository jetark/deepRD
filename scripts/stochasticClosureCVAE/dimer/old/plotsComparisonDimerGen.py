...
redModelfnamebase[6] += '_pipimririm/simMoriZwanzigReduced_'
redModelfnamebase[7] += '_piririm/simMoriZwanzigReduced_'

# Create plot directory
plotDirectory = os.environ['DATA'] + 'stochasticClosure/dimer/boxsize' + str(bsize) + '/plotsCVAE/'
try:
...
conditionedList = ['pipimririm']  # ['pi', 'piri', 'piririm']
trajIndexes = []
...
if 'pipimririm' in conditionedList:
...
labelList = [r'$\tilde{r}^{n+1}|\tilde{r}^n$', r'$\tilde{r}^{n+1}|\tilde{r}^n, \tilde{r}^{n-1}$',
             ...
             r'$\tilde{r}^{n+1}|\tilde{v}^n$', r'$\tilde{r}^{n+1}|\tilde{v}^n,\tilde{v}^{n-1}, \tilde{r}^n$,\tilde{r}^{n-1}',
